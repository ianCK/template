...
int main() {
  ...
  for (int i = 1; i < n; i++) {
    MakeRoot(pool + edge[i].first);
    Link(
      pool + edge[i].first, pool + edge[i].second);
  }
  int root;
  cin >> root;
  MakeRoot(pool + root);
  while (m--) {
    ...
    switch (op) {
      case 0:
        ...
        fg = Flag(0, y);
        pool[x].val = y;
        if (pool[x].c[2]) pool[x].c[2]->UpdSub(fg);
        if (pool[x].c[3]) pool[x].c[3]->UpdSub(fg);
        pool[x].Pull();
        break;
      case 1:
        cin >> x;
      ...
      case 3:
        ...
        ans = pool[x].val;
        if (pool[x].c[2])
          ans = min(ans, pool[x].c[2]->all.mn);
        if (pool[x].c[3])
          ans = min(ans, pool[x].c[3]->all.mn);
        cout << ans << '\n';
        break;
      case 4:
        cin >> x;
        ...
        ans = pool[x].val;
        if (pool[x].c[2])
          ans = max(ans, pool[x].c[2]->all.mx);
        if (pool[x].c[3])
          ans = max(ans, pool[x].c[3]->all.mx);
        cout << ans << '\n';
        break;
      case 5:
        cin >> x >> y;
      ...
      case 7:
        cin >> x >> y;
        cout << QueryLca(pool + x, pool + y).mn
             << '\n';
        break;
      case 8:
        cin >> x >> y;
        cout << QueryLca(pool + x, pool + y).mx
             << '\n';
        break;
      case 9:
        cin >> x >> y;
      ...
      case 10:
        cin >> x >> y;
        cout << QueryLca(pool + x, pool + y).sum
             << '\n';
        break;
      case 11:
        cin >> x;
        ...
        ans = pool[x].val;
        if (pool[x].c[2])
          ans += pool[x].c[2]->all.sum;
        if (pool[x].c[3])
          ans += pool[x].c[3]->all.sum;
        cout << ans << '\n';
        break;
    }
  }
