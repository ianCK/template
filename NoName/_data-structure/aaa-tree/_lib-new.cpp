...
struct Info {
  int mx, mn, sum = 0, sz = 0;
  Info() : mx(-kInf), mn(kInf) {}
  Info(int mx_, int mn_, int sum_, int sz_)
      : mx(mx_), mn(mn_), sum(sum_), sz(sz_) {}
};
struct Flag {
  int mul = 1, add = 0;
  Flag() = default;
  Flag(int mul_, int add_)
      : mul(mul_), add(add_) {}
...
Node *NewNode() {
  ...
  x->all = x->chain = Info();
  x->inner = true, x->rev = false, x->val = 0;
  return x;
}
