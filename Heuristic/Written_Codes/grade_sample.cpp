...
int main() {
	...
	for (int i = 1; i <= cases; i++) {
		...
		System("sol < sample_in/" + my_to_string(kLen, i) + ".txt > sample_out/out" + my_to_string(kLen, i));
	}
	printf("\n");

	printf("calculating...\n");
	for (int i = 1; i <= cases; i++) System("grade sample_in/" + my_to_string(kLen, i) + ".txt sample_out/out" + my_to_string(kLen, i) + " > score" + my_to_string(kLen, i));
