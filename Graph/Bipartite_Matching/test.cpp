...
#include "C:\Users\ianli\Desktop\CP\template\Graph\Bipartite_Matching\Bipartite_Matching.cpp"
...
int main() {
	...
	bm.init(l, r);
	for (int i = 1; i <= m; i++) bm.AddEdge(a[i], b[i]);

	printf("%d\n", bm());
	for (int i = 0; i < l; i++) if (~bm.matchx[i]) printf("%d %d\n", i, bm.matchx[i]);
}
