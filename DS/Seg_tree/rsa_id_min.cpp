...
template <typename T> struct seg_tree_rsa_id_min {
	...
	void pull(int n) {
		_val[n] = min(_val[n << 1], _val[n << 1 | 1]);
		return ;
	}

	void init(int n, int l, int r) {
		_val[n] = make_pair(kInf, l);
		_has_set_flag[n] = false;
		_add_flag[n] = 0;
		if (l < r) {
			int mid = (l + r) >> 1;
	...
	pair<T, int> ask(int n, int l, int r, int L, int R) {
		...
		else if (l > R || L > r) return make_pair(kInf, l);
		else {
			int mid = (l + r) >> 1;
			push(n);
	...
	public:
	seg_tree_rsa_id_min() : _size(0), _val(nullptr), _add_flag(nullptr), _set_flag(nullptr) {}
	void init(int n) {
