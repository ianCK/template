...
#include "C:\Users\ianli\Desktop\CP\template\Various\Multi_Array\arr.cpp"

int main() {
	Arr3<int> arr(5, 5, 5);

	int n = 4;
	for (int i = 1; i <= n; i++) for (int j = 1; j <= n; j++) for (int k = 1; k <= n; k++) RP(arr[i][j][k]);
	
	for (int i = 1; i <= n; i++) {
		printf("i = %d\n", i);

		for (int j = 1; j <= n; j++, printf("\n")) for (int k = 1; k <= n; k++) printf(" %d", arr[i][j][k]);
