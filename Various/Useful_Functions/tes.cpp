...
template <typename T> void mv_inner(vector<T> &vec, T c) {return vec.PB(c);}
template <typename... T> void mv_inner(vector<T> &vec, T c, T... targs) {
	vec.PB(c);
	return mv_inner(vec, targs...);
}
template <typename... T> vector<T> mv(T... targs) {
	vector<T> vec;
	mv_inner(vec, targs...);
	return vec;
}
