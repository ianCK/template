...
template <typename T, T (*op)(const T&, const T&)> constexpr Subgroup<T, op> Subgroup<T, op>::operator * (const Subgroup<T, op>& o) const { return Subgroup<T, op>(op(val, o.value())); }
